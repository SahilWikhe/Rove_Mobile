...
import { requestTrackingPermissions, unblockTracking, synchronizeBackgroundTracking, stopBackgroundTracking } from '../tracking/background';
...
export default function Drive() {
  const { api, synthetic, profile: account } = useSession(); const [profile, setProfile] = useState<DriverProfile | null>(null);
  ...
  async function availability(confirmed = false) {
    if (!profile) return;
    if (!profile.online && !synthetic && !confirmed) { setExplainLocation(true); return; }
    setExplainLocation(false); setBusy(true); setError(null);
    try {
      ...
      if (!synthetic && account) {
        if (updated.online) { await unblockTracking(); await synchronizeBackgroundTracking(api, account.id); }
  ...
  return <Screen><Stack.Screen options={{ headerShown: false }} /><Brand driver />{synthetic && <Banner message="Synthetic test mode · no real rides or payments" />}<Copy kind="title">{profile?.online ? 'You’re online.' : 'Ready when you are.'}</Copy>{error && <Banner error message={error} />}{trackingError && <Banner error message={trackingError} />}
    ...
    {explainLocation && <Card><Copy kind="heading">Keep your ride connected.</Copy><Copy>Rove needs precise location while you’re online, including when the phone is locked or you’re using navigation. Go offline to stop sharing. On the next screen, choose Always / Allow all the time.</Copy><Button title="Continue with location" loading={busy} onPress={() => void availability(true)} /><Button title="Not now" variant="secondary" onPress={() => setExplainLocation(false)} /></Card>}
    {trackingError && profile?.online && !synthetic && <Button title="Reconnect location" variant="secondary" loading={busy} onPress={() => {
      if (!account) return;
      setBusy(true);
      void requestTrackingPermissions().then(unblockTracking).then(() => synchronizeBackgroundTracking(api, account.id))
        .catch(failure => setError(failure instanceof Error ? failure.message : 'Location could not reconnect.')).finally(() => setBusy(false));
    }} />}
