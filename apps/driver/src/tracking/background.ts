...
function client(grant: StoredGrant) { return new ApiClient(API_URL, async () => grant.token); }
async function stop(grant: StoredGrant | null, revoke: boolean) {
  ...
  if (grant && revoke) {
    try { await client(grant).request('/tracking/v1/session', z.object({ revoked: z.boolean() }), { method: 'DELETE' }); }
    catch { /* Locally stopped; server also enforces expiry/offline/disabled checks. */ }
  }
}
...
if (Platform.OS !== 'web' && !TaskManager.isTaskDefined(TASK)) {
  TaskManager.defineTask<{ locations: Location.LocationObject[] }>(TASK, async ({ data, error }) => {
    ...
    try { await serial(async () => {
      ...
      if (!sample) return;
      try {
        await client(grant).request('/tracking/v1/location', z.object({ accepted: z.boolean() }), { method: 'POST', body: sample });
      } catch (failure) {
...
export async function requestTrackingPermissions() {
  ...
  if (!await TaskManager.isAvailableAsync()) throw new Error('Background tracking requires an installed driver development or release build.');
  const foreground = await Location.requestForegroundPermissionsAsync();
  if (foreground.status !== 'granted') throw new Error('Allow precise location in Settings to go online.');
  const background = await Location.requestBackgroundPermissionsAsync();
  if (background.status !== 'granted') throw new Error('Choose Always / Allow all the time in Location Settings before going online.');
}
...
export function synchronizeBackgroundTracking(api: ApiClient, driverId: string) {
  return serial(async () => {
    ...
    if (await SecureStore.getItemAsync(BLOCKED)) throw new Error('Location session ended. Tap Reconnect location to resume on this device.');
    const permission = await Location.getBackgroundPermissionsAsync();
    if (permission.status !== 'granted') {
      await stop(grant, true);
      throw new Error('Background location is unavailable. Restore Always / Allow all the time access in Settings.');
    }
    if (!grant || grant.driverId !== driverId || Date.parse(grant.expiresAt) < Date.now() + 60 * 60 * 1000) {
      ...
      await SecureStore.setItemAsync(KEY, JSON.stringify(grant), { keychainAccessible: SecureStore.AFTER_FIRST_UNLOCK_THIS_DEVICE_ONLY });
    }
