...
if (!filename || process.argv.length !== 3) {
  ...
  try {
    ...
    if (!result.valid) {
      ...
      console.log('Staging configuration shape passed. No network requests or deployment performed.');
      console.log(
    ...
    console.error('Unable to read staging environment file. No settings changed.');
    process.exitCode = 1;
  }
}
