...
interface Dependencies {
  pool: Pool; verifyIdentity: VerifyIdentity; rides: RideService; quotes: QuoteService; maps: MapsProvider;
...
async function body<T>(c: Context, schema: z.ZodType<T>): Promise<T> {
  if (!c.req.header('content-type')?.toLowerCase().startsWith('application/json')) throw new DomainError('INVALID_BODY', 'Send a JSON request.', 400);
  try { return schema.parse(await c.req.json()); }
  catch { throw new DomainError('INVALID_BODY', 'Check the request fields and try again.', 400); }
}
...
export function createApp(deps: Dependencies) {
  ...
  app.use('*', bodyLimit({ maxSize: 32_768, onError: c => c.json({ error: { code: 'BODY_TOO_LARGE', message: 'Request is too large.', requestId: c.var.requestId } }, 413) }));
  app.onError((error, c) => {
    const known = error instanceof DomainError;
    return c.json({ error: { code: known ? error.code : 'INTERNAL_ERROR', message: known ? error.message : 'Something went wrong. Please try again.', requestId: c.var.requestId } }, known ? error.status : 500);
  });
  ...
  app.use('/v1/*', async (c, next) => {
    ...
    const identity = await deps.verifyIdentity(match[1]); c.set('subject', identity.subject);
    const result = await deps.pool.query<{ id: string; role: Actor['role']; disabled: boolean }>('SELECT id,role,disabled FROM users WHERE subject=$1', [identity.subject]);
    const user = result.rows[0];
    if (user?.disabled) throw new DomainError('ACCOUNT_DISABLED', 'Contact support for help with your account.', 403);
    if (user) c.set('actor', { id: user.id, role: user.role });
    else if (!(c.req.path === '/v1/me' && c.req.method === 'POST')) throw new DomainError('PROFILE_REQUIRED', 'Complete your profile to continue.', 403);
    await next();
  });
  app.post('/v1/me', async c => {
    const input = await body(c, z.object({ name: z.string().trim().min(1).max(100), role: z.enum(['rider', 'driver']) }).strict());
    ...
    const result = await deps.pool.query<{ id: string; role: Actor['role']; name: string }>(
      'INSERT INTO users (subject,name,role) VALUES ($1,$2,$3) ON CONFLICT (subject) DO UPDATE SET subject=EXCLUDED.subject RETURNING id,role,name', [c.var.subject, input.name, input.role]);
    const user = result.rows[0]!;
    if (user.role === 'driver') await deps.pool.query('INSERT INTO drivers (id) VALUES ($1) ON CONFLICT DO NOTHING', [user.id]);
    return c.json(user);
  });
  app.get('/v1/me', async c => {
    const user = (await deps.pool.query('SELECT id,name,role FROM users WHERE id=$1', [c.var.actor.id])).rows[0];
    return c.json(user);
  });
  app.get('/v1/me/capabilities', async c => {
    let flags = { scheduling: false, weekly: false, monthly: false };
    try { flags = await deps.flags(); } catch { /* New feature access fails closed. */ }
    return c.json(capabilities(flags, new Date()));
  });
  app.get('/v1/places', async c => {
    const q = c.req.query('q')?.trim() ?? '';
    if (q.length < 3 || q.length > 150) throw new DomainError('INVALID_QUERY', 'Enter at least three characters.', 400);
  ...
  app.put('/v1/drivers/me/availability', async c => {
    ...
    return c.json(await drivers.availability(c.var.actor, input.online, input.coordinate, c.req.header('Idempotency-Key') ?? ''));
  });
  app.post('/v1/drivers/me/heartbeat', async c => c.json(await drivers.heartbeat(c.var.actor, await body(c, Heartbeat))));
  app.get('/v1/drivers/me/offers', async c => c.json(await drivers.offers(c.var.actor)));
  app.post('/v1/offers/:id/decline', async c => {
    await body(c, z.object({}).strict());
    return c.json(await drivers.decline(c.var.actor, id(c.req.param('id')), c.req.header('Idempotency-Key') ?? ''));
  });
  app.get('/v1/rides', async c => c.json(await listRides(deps.pool, c.var.actor, c.req.query('before') ? id(c.req.query('before')!) : undefined)));
  ...
  app.post('/v1/ride-requests', async c => {
    ...
    return c.json(await deps.rides.request(c.var.actor, input.quoteId, c.req.header('Idempotency-Key') ?? ''), 201);
  });
  app.post('/v1/offers/:id/accept', async c => {
    await body(c, z.object({}).strict());
    return c.json(await deps.rides.accept(c.var.actor, id(c.req.param('id')), c.req.header('Idempotency-Key') ?? ''));
  });
  app.post('/v1/rides/:id/transitions', async c => {
    const input = await body(c, z.object({ state: RideState, expectedVersion: z.number().int().positive() }).strict());
    return c.json(await deps.rides.transition(c.var.actor, id(c.req.param('id')), input.state, input.expectedVersion, c.req.header('Idempotency-Key') ?? ''));
  });
  return app;
}
