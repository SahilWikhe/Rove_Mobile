...
export function oidcIdentity(config: { issuer: string; audience: string; jwksUrl: string }, testKeys?: JWTVerifyGetKey): VerifyIdentity {
  if (![config.issuer, config.jwksUrl].every(value => new URL(value).protocol === 'https:')) throw new Error('Identity provider URLs must use HTTPS.');
  if (!config.audience) throw new Error('An API audience is required.');
  const keys = testKeys ?? createRemoteJWKSet(new URL(config.jwksUrl), { timeoutDuration: 5000, cooldownDuration: 30_000 });
  return async token => {
    try {
    ...
    } catch { throw new DomainError('UNAUTHENTICATED', 'Please sign in again.', 401); }
  };
}
