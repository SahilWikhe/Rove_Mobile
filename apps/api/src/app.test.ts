...
const place = { id: 'synthetic-place', label: 'Synthetic pickup', area: 'Raleigh fixture', coordinate: { latitude: 35.8, longitude: -78.6 } };
const maps: MapsProvider = { search: async () => [place], resolve: async () => place, route: async () => ({ distanceMeters: 5000, durationSeconds: 720 }) };
const riderId = randomUUID();
beforeAll(async () => {
  database = await testDatabase();
  app = createApp({ pool: database.pool, rides: new RideService(database.pool),
    quotes: new QuoteService(database.pool, maps, developmentRates, { south: 35, north: 37, west: -80, east: -77 }), maps,
    verifyIdentity: async token => { if (!['rider', 'new-user', 'driver'].includes(token)) throw new DomainError('UNAUTHENTICATED', 'Please sign in.', 401); return { subject: token }; },
    flags: async () => { throw new Error('Provider unavailable'); },
  });
}, 60_000);
afterAll(async () => { await database?.close(); });
beforeEach(async () => { await database.pool.query('TRUNCATE users CASCADE'); await database.pool.query('TRUNCATE outbox'); await database.db.insert(users).values({ id: riderId, subject: 'rider', name: 'Test rider', role: 'rider' }); });
function request(path: string, input?: unknown, token = 'rider', key = randomUUID()) {
  return app.request(path, { method: input === undefined ? 'GET' : 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json', 'Idempotency-Key': key }, ...(input === undefined ? {} : { body: JSON.stringify(input) }) });
}
test('health is public while protected endpoints require authentication', async () => {
  ...
  for (const path of ['/v1/me', '/v1/me/capabilities', '/v1/places?q=Raleigh']) expect((await app.request(path)).status).toBe(401);
...
test('signup cannot grant staff or overwrite an existing role', async () => {
  ...
  expect((await (await request('/v1/me', { name: 'Change role', role: 'driver' })).json()).role).toBe('rider');
});
test('quotes reject fare injection and use provider-resolved place data', async () => {
  const input = { pickup: { ...place, label: 'Spoofed', coordinate: { latitude: 0, longitude: 0 } }, destination: place, service: 'standard' };
  expect((await request('/v1/quotes', { ...input, fare: 1 })).status).toBe(400);
  const response = await request('/v1/quotes', input); expect(response.status).toBe(201);
  const quote = await response.json(); expect(quote.pickup).toEqual(place); expect(quote.fare.amount).toBe(1050);
  ...
  expect(first.status).toBe(201); expect(await first.json()).toEqual(await second.json());
});
...
test('flag outages fail closed and responses never cache personal data', async () => {
  ...
  expect(await response.json()).toMatchObject({ scheduleCreate: false, scheduleWeekly: false, scheduleMonthly: false });
});
test('malformed bodies and resource ids return safe errors with request ids', async () => {
  const response = await app.request('/v1/quotes', { method: 'POST', headers: { Authorization: 'Bearer rider', 'Content-Type': 'application/json' }, body: '{' });
  expect(response.status).toBe(400);
  expect(await response.json()).toMatchObject({ error: { code: 'INVALID_BODY', requestId: expect.any(String) } });
  expect((await request('/v1/offers/not-a-uuid/accept', {})).status).toBe(400);
  const oversized = await request('/v1/quotes', { text: 'x'.repeat(40_000) }); expect(oversized.status).toBe(413);
});
test('unassigned drivers cannot read rides and exact details are removed after the trip ends', async () => {
  ...
  const quote = await (await request('/v1/quotes', { pickup: place, destination: place, service: 'standard' })).json();
  ...
  await database.pool.query("UPDATE rides SET driver_id=$2,state='matched' WHERE id=$1", [ride.id, driver.id]);
  const assigned = await (await request(`/v1/rides/${ride.id}`, undefined, 'driver')).json();
  expect(assigned.pickup).toEqual(place); expect(assigned.rider).toEqual({ name: 'Test rider' });
