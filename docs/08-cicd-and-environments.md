...
Start `node scripts/native-message-peer.mjs 8190` on a fresh synthetic database. It watches only for the test marker from the native driver, waits five seconds beyond observing the persisted send, then creates one reply as the synthetic rider. It exits after sending or fails after ten minutes. This is a local test counterpart, not a deployed bot or real user account. Stop it if the native flow fails before sending the marker.

Run the Android trip runner with `NATIVE_TRIP_MESSAGES=1 ACCOUNT_API_PORT=8193`. The shared base ride flow includes `native-tests/complete-trip-messaging.yaml`: open Message rider, send with the native composer, assert the delayed reply while the thread stays open, capture the screen and return to the ride. Keep the proxy log and peer timestamp alongside Maestro/JUnit evidence. A message visible after reopening alone does not prove realtime delivery; require a ready native WebSocket, its message-change event after the peer send, and the foreground UI assertion in the same run. The test does not establish locked-phone/background delivery, push, or native rider-side receiving behavior.
