...
- Existing Stripe credentials and mode must match the deployment. A separate `STRIPE_CONNECT_WEBHOOK_SECRET` for the thin-event endpoint is also required. Prefer restricted keys with only the required Connect operations enabled.
...
Account-event ingestion, reconciliation, bounded eligibility and capability revocation are now implemented; see [account events](57-payout-account-reconciliation.md). Real event delivery, transfers and payout recovery, dashboard access, refunds/disputes and native accessibility/visual checks remain delivery requirements. The future connected-account management surface should include Stripe's notification banner so changing requirements remain visible. Until these gates are implemented and verified, an account reporting ready on this screen is not a production launch approval.
...
## Sandbox recipient and link verification — September 13

Applied migration 0089 to provider staging and verified all 48 tables retain enabled/forced RLS and restricted runtime grants. The existing synthetic reservation had failed provider validation before creation. A complete paginated Accounts v2 inventory found no account matching its driver/binding metadata; the private verifier supplied the missing test contact to that exact reservation while preserving its original binding and idempotency key. Corrected creation succeeded through DriverPayouts and StripeDriverPayouts using rove_staging_app.

Provider status returned needs_information. Onboarding links passed the existing host/mode/account/expiration contract. Reopening setup reused the same account and local contact_email was null after mapping. Driver approval stayed false. The single synthetic recipient and private fixture remain for hosted onboarding acceptance. No transfers, bank payouts, production changes or hosted feature activation occurred. This is actual provider/restricted-service evidence, not authenticated native onboarding completion or active payout capabilities.
