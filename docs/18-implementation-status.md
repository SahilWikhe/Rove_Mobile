...
Sonar run 34796009452 passed for 34fc7c839e25a1c99818a81cbe6e928eb9d1db7c. The same candidate's CI quality, tests, security, infrastructure, mobile, CodeQL and browser and native Android rider jobs passed; three native jobs were still running. No physical device was connected. Work remains limited to the owner's four priorities: payout acceptance, complete native ride acceptance, physical-device tracking/push, and production/staff/recovery/final-candidate release checks. Auth0 tenant inspection could not proceed because the CLI session expired. Production mobile configuration remains unfilled, and the expected production-candidate/production-promotion GitHub environments were absent on read-only inspection. These remain within the existing launch-setup gate. Documentation lint and diff checks passed. Next: finish the existing native CI run and resume provider/device/staff checks when their pending access and approval dependencies are resolved. No new feature scope or production activation.
...
Restricted-role tests exercise concurrency, source/event mismatch, terminal-ride evidence, permission/MFA/revocation, pagination, immutable fields, forced RLS/context reset and recovery. The recovery test exposed identity binding clearing the outbox read scope; the exact event scope is now bound after permission verification and the regression rerun. Initial synthetic fixture UUID/text casts were corrected. API route tests cover authentication, permissions, strict input and no-store responses. Final verification passed 10 focused server tests, all 215 API tests and all 13 database tests; API/server/database typechecks, changed-source lint/format, import boundaries, all 91 Markdown files and diff checks passed. No hosted migration, feature activation, staff dashboard UI or real MFA acceptance is claimed. Next: migrate and verify this intake in staging, then remaining native/provider/device/recovery release work. Production remains incomplete; remote push confirmation follows separately.
...
Before/after configuration checks confirm refund operations/accounting, Connect onboarding/transfers, push and document cleanup remain disabled. Private evidence excludes credentials and fixture identifiers from Git. This proves the hosted receipt API, not native rendering, staff refund authorization or the complete booking lifecycle. Current observed CI run 34785366398 has passed all jobs except rider iOS, which remains running; no current-main green claim. Documentation checks run before the authorized push. Next: finish current native CI verification and remaining payment/device/recovery acceptance; production is still incomplete. Push confirmation follows separately.

## Provider-backed sandbox payment passed; missing notification consumer fixed — September 13

Verified existing sandbox identity and test key, then used only dedicated synthetic staging accounts for a persisted $2 in-progress fixture, avoiding route lookup/matching. Actual payment/customer/reconciliation adapters ran through rove_staging_app. Stripe manual authorization and capture succeeded; a concurrent hosted webhook caused the expected revision retry, with readback already paid. Resuming the exact saved intent/attempt/keys retained one balanced capture and allocation. A full $2 refund and same-key retry returned one refund ID; refund reconciliation produced one balanced refund-balance journal. Hosted creation/capturable/success webhooks each persisted and their three reconciliation jobs completed; one capture job completed. Read-only original balance inspection returned 200 gross/36 fee/164 net cents, pending funding and zero unrefunded cents. The terminal fully refunded fixture and immutable financial history remain; private references are outside Git.

The test exposed three dead-lettered payment.updated events with no consumer. Source now includes version/paymentState in the durable event and registers rider-only generic billing hints for current paid/action-required/review-required states. Routine, stale, expired and malformed legacy events are suppressed. Disabled push explicitly pauses these jobs without attempts/leases/dead letters or drain hot-loop; financial jobs continue. Existing hosted dead letters were not changed and push remains off.

Verification: 51 server payment/audience/delivery tests, all 213 API tests, server/API typechecks pass. A new test initially used an incorrect receipt-state name; corrected to the existing receipt enum and reran successfully. Tests cover runtime off/on behavior, actual outbox dispatch/idempotency, payload provenance, rider-only ownership, changed state/version and routine-state suppression. Changed-file lint/format and diff checks pass; all 90 Markdown files pass after correcting a duplicate blank line. Accounts v2 listing returned 403 with the staging key; recipient provisioning/transfers remain unverified and disabled. Native UI/3DS, full booking, staff refund HTTP authorization, bank payouts and external restore/erasure recovery remain release gates. CI 856d718 has passed both Android jobs and driver iOS; rider iOS remains running. Next: verify refreshed hosted code/CI and continue device, Connect and recovery acceptance. Prepared for authorized main push; confirmation follows separately.

## Local synthetic logical backup/restore verified — September 13

Added db:restore:rehearsal with PostgreSQL 18 pg_dump/pg_restore/psql, using only a new embedded local server and synthetic records. It accepts no hosted target, clears ambient PostgreSQL client settings, creates a custom archive, deliberately changes source data after backup and restores into an empty database. Canonical schema comparison includes policies/grants/ownership; all public/drizzle records match the pre-backup state. PostgreSQL reparses equivalent nested constraints, so an independent plain-schema baseline supplies canonical comparison without stripping SQL expressions. Runtime checks use a genuine non-owner NOSUPERUSER/NOBYPASSRLS login.

The final command passed with 47 enabled/forced application tables, 88 migration entries, restored synthetic user records and denied unrelated access. A fault-injected restored record caused failure and exit 1 after cleanup; the good run exited 0. Fixed explicit exit handling after finding the embedded server exit hook could mask a failed assertion. Temporary archives/server were cleaned; no hosted database or paid service was used. Local libpq 18.6 is installed. Most tables in this fixture are empty and roles share one local cluster: this does not establish provider-backed financial recovery, external closure/erasure replay or Neon restoration. The recovery runbook now provides the repeatable command and these limits.

Current observed CI on 856d718 has passed tests, browser, quality, security, infrastructure, CodeQL, mobile exports and driver Android. Rider Android and both iOS jobs remain running; latest main evidence is still pending. Unexpected target arguments were rejected before database creation with exit 1. Changed-file lint/format, workspace boundaries and all 89 Markdown files pass after correcting a duplicate blank line; diff checks pass before the authorized main push. Next: finish hosted native verification and continue external recovery/device/provider acceptance. Production remains incomplete; remote push confirmation follows separately.
...
Added backup-recovery.md with verified inventory, isolated restore steps, RLS/journal/data checks, external payment/identity/storage reconciliation, required synthetic failure scenarios and production cutover requirements. It explicitly identifies the independent post-restore closure/hold/erasure record and automated deletion replay as unimplemented/unverified, rather than treating a restored database as safe to serve. Paid backup setup remains deferred under the owner's instruction. Production setup links this acceptance procedure. Verification for this documentation checkpoint consists of actual provider metadata and official recovery docs, plus Markdown/diff validation before commit; no restore was performed. Next: current CI/Sonar verification, device/provider acceptance and recovery implementation/rehearsal after the required setup. Prepared for authorized main push; remote confirmation follows.
...
Two fresh local ARM64 emulator runs using the existing driver release APK reproduced the same Maestro failure despite readiness checks. The second run captured state before cleanup: emulator exit/signal were null, adb get-state returned device and sys.boot_completed remained 1. Thus readiness changes alone are not a verified fix; investigation shifts to Maestro transport behavior. The runner now preserves this scoped failure-state JSON in CI artifacts without restarting ADB or touching other emulators. Both runs ended and their temporary AVDs were cleaned up. Local cached APK tests do not validate a new CI binary. Changed-runner formatting/lint and all three native-smoke-command tests passed. Docs validation is rerun for this checkpoint. Latest CI run 34782892925 remained in progress. Next: isolate Maestro transport compatibility while preserving the full release launch/relaunch gate; physical-device/provider release work also remains. Prepared for authorized main push; remote confirmation follows.
...
Actual-policy testing exposed pre-authorization owner lookups and related-account dependencies. Fixed document intake/review/storage, driver eligibility, payment session rechecks, ride acceptance, reviewed message cleanup and closure identity resolution. Closure lookup is read-only, limited to one request, cleared immediately, then followed by the existing disabled-account/retention checks. Removed the users-policy prototypes from tracking, notification and profile tests. Signup now runs through a restricted runtime role in its integration tests. Trigger tests verify both unscoped denial and authorized retention/closed-account safeguards.

All 759 server, 208 API and 13 database tests passed, with server/database typechecking, changed-source lint and diff checks. Initial users RLS failures reduced from 112 to zero through compatible access fixes; no policy gate or assertion was bypassed. Source coverage reaches 45/47; isolated hosted remains 44/47 through 0081. Drivers and rides remain. The future rides policy must avoid referring back to users, since users related-party reads depend on rides. Next: isolated hosted migration/rehearsal, remaining core policies and compatible provider-staging rollout. Physical-device/provider acceptance remains incomplete. This checkpoint is prepared for the authorized main push; remote confirmation follows.
...
Replaced fixed PostgreSQL test passwords in the disposable database helper and backend build verifier with cryptographically generated per-run values. The connection-close test now receives the generated test connection string instead of assuming a fixed password. The first test run exposed that dependency; after correction, all 1,158 tests passed with coverage, along with backend build/verification, changed-file lint and formatting. This is a synthetic-test hardening change, not a production credential rotation. A documentation lint check caught one extra blank line; that formatting issue was corrected and documentation validation rerun. Scanner confirmation remains pending.

Next: inspect and address proxy forwarding and CLI file-path findings, review workflow/tooling alerts, then resume the ten remaining core-table RLS policies and provider/device acceptance. The audit-policy refactor was saved outside the worktree before this security review; it is not implemented or enabled.
...
SonarQube attempt 2 completed analysis after the user disabled Automatic Analysis. Its gate failed only because new-code coverage was 0% without an uploaded report (80% required); the other five conditions passed. The workflow now runs the six existing Vitest suites with V8 LCOV reporting before scanning, using repository-relative paths and including unexecuted source files. All six local coverage suites and dependency typechecks passed (11 Turbo tasks); server line coverage is 92.26%. LCOV source paths were checked against repository files. Formatting, lint, docs and actionlint passed. The next hosted gate is pending. No coverage threshold was weakened. PostgreSQL parser warnings remain a scanner limitation, covered separately by actual migration/database tests.

Next: verify and publish coverage reporting, then continue the ten remaining core-table RLS policies and compatible provider-staging rollout. Physical-device/provider acceptance and production readiness remain incomplete.
