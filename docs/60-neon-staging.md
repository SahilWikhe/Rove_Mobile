...
The first expanded rehearsal correctly hit ACCOUNT_CLOSURE_HOLD because a payment fixture left driver earnings payable on the document-cleanup driver. The final passing run used a separate synthetic payment driver, preserving the closure safeguard. Provider staging was separately inspected read-only and remains at 31 migrations, 32 tables and zero RLS-enabled/forced tables. Production was not changed. Migrations 0067–0068 are now included in the passing hosted rehearsal. Source migrations 0069–0070 additionally protect refund authorizations/execution and loss-allocation decisions with local verification. Seventeen current-schema tables still need policies, and compatible API/worker deployment must precede the reviewed provider-staging migration delta. Earlier isolated-rehearsal coverage and pending-verification notes below are historical.
