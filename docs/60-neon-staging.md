...
The isolated synthetic rehearsal branch has all 67 migrations through 0066. Live catalog inspection confirms twenty-four of 47 tables have RLS enabled and forced. Hosted checks through pooled rove_staging_app (NOSUPERUSER/NOBYPASSRLS) passed for customer provisioning retries, payment reconciliation and capture accounting retries, readiness mutation denial, worker customer isolation and staff permissions. The existing request limits, installation transfer, push send/receipt retries, messaging, vehicle review, retention, closure and document upload/scan/review/approved cleanup workflows passed again. All external adapters were synthetic; no real provider operation occurred. The temporary branch expires September 14.

The first expanded rehearsal correctly hit ACCOUNT_CLOSURE_HOLD because a payment fixture left driver earnings payable on the document-cleanup driver. The final passing run used a separate synthetic payment driver, preserving the closure safeguard. Provider staging was separately inspected read-only and remains at 31 migrations, 32 tables and zero RLS-enabled/forced tables. Production was not changed. Source migrations 0067–0068 additionally protect refund/dispute checks and observation histories with local verification. Nineteen current-schema tables still need policies, and compatible API/worker deployment must precede the reviewed provider-staging migration delta. Earlier isolated-rehearsal coverage and pending-verification notes below are historical.
