...
The isolated synthetic rehearsal branch has all 64 migrations through 0063. Live catalog inspection confirms twenty-one of 47 tables have RLS enabled and forced. Hosted restricted pooled-role checks passed for concurrent request limits, same-phone device account transfer, idempotent notification fanout/send/receipt handling, and unscoped counter/delivery/installation denial. Existing saved places, vehicle review, messaging/read notifications, support-backed consent, inventory, retention, closure and document upload/scan/review/approved cleanup workflows passed again. All external adapters were synthetic: no real push delivery, storage removal, external identity erasure or payments occurred. The temporary branch expires September 14.

Provider staging was separately checked read-only and remains at 31 migrations, 32 tables and zero RLS-enabled tables. Earlier pending isolated-rehearsal statements below are superseded by this result. No provider-staging or production migration has occurred. Source migrations 0064–0066 additionally protect staff permissions, capture-accounting checks and payment-customer mappings with local verification. Twenty-three current-schema tables still need policies, and compatible backend/worker deployment must precede the reviewed staging migration delta.
