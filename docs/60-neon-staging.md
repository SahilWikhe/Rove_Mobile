...
The isolated synthetic rehearsal branch has all 60 migrations through 0059. Live catalog inspection confirms seventeen of 47 tables have RLS enabled and forced. Hosted restricted pooled-role checks passed for saved places, vehicle review, messaging/read notifications, support-backed consent, inventory, closure, retention enforcement/release and reviewed message cleanup. The six newest tables passed document reservation/upload, receipt isolation, scanner result, staff review, closure, cleanup approval and exact-version removal/retry checks. All six denied unscoped reads. These checks used synthetic data and provider adapters, with no real storage removal, push delivery, external identity erasure or payments. The temporary branch expires September 14.

Provider staging was separately checked read-only and remains at 31 migrations, 32 tables and zero RLS-enabled tables. The implementation and rehearsal sections below record earlier checkpoints; their pending isolated-rehearsal statements are superseded by this result. No provider-staging or production migration has occurred. Source migrations 0060–0062 additionally protect request counters, push counters and delivery records with local verification; twenty-seven current-schema tables still need policies, and compatible backend/worker deployment must precede the reviewed staging migration delta.
