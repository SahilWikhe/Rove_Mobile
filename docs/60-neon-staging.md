...
Provider staging was separately checked read-only and remains at 31 migrations, 32 tables and zero RLS-enabled tables. Earlier pending isolated-rehearsal statements below are superseded by this result. No provider-staging or production migration has occurred. Source migrations 0064–0065 additionally protect staff permissions and capture-accounting checks with local verification. Twenty-four current-schema tables still need policies, and compatible backend/worker deployment must precede the reviewed staging migration delta.
