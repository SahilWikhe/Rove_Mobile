...
The disposable `rove-rls-rehearsal-20260913` branch (`br-shy-bar-axjulxqh`) has 88 migrations through 0087. Live catalog inspection on September 13 confirms all 47 public tables have RLS enabled and forced. The complete hosted rehearsal passed through pooled `rove_staging_app` with NOSUPERUSER/NOBYPASSRLS, including booking, ride history/cancellation, receipt/earnings APIs, signup/profile isolation and the existing synthetic payment, tracking, messaging, notification, vehicle, closure and document-cleanup workflows. All external provider adapters remained fake. The branch expires September 14.

Provider staging was subsequently migrated through 0087 after confirming the compatible e5c9888 deployment on its stable alias. Live catalog now confirms 88 migrations and all 47 public tables enabled/forced. Existing runtime grants were refreshed without password rotation. Read-only runtime permission/policy checks and unscoped core-table denial passed; hosted health returned 200 and unauthenticated profile access returned 401. Production was not changed. Next: authenticated sandbox-provider verification. Hosted synthetic success does not prove physical-device or production acceptance. Older policy counts and pending-verification notes below are historical checkpoints.

## Current environment selection

Hosted provider staging uses `rove-provider-staging`, endpoint `ep-lucky-bar-ax4k1m1t.c-4.us-east-2.aws.neon.tech`, and ignored `.staging-provider/.env.neon.staging`. It now has 88 migration entries through `0087`; all 47 public tables have enabled/forced RLS. The realtime location trigger was verified previously. It now contains dedicated staging integration records, so the original empty-table observation is historical. Normal API queries use the pooled restricted `rove_staging_app` role; realtime uses a direct connection with that same role/database.
