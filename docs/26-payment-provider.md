...
Mapped customer/attempt records and capture/release/reconciliation handlers are implemented in docs/28-payment-reconciliation.md. Durable intent creation and owner-only session APIs are implemented in docs/29-payment-session-creation.md. Remaining work: customer/method provisioning, ledger/receipts/earnings, complete booking/payment UI, native PaymentSheet, production worker composition, refund authorization, periodic reconciliation, sandbox acceptance tests, native UI and Connect/payout setup. The existing local synthetic settlement handlers remain clearly labeled fixtures until the real domain is wired. Do not enable production bookings based on this adapter alone.
