...
Local restricted-role, full server, API and migration tests passed. Migration 0057 additionally enables/forces reservation RLS: active owners access their documents, current MFA staff have permission-scoped reads and locks without writes, scanners read quarantined queue entries or an exact target, and cleanup workers read their assigned owner's documents. Migration 0058 protects scan records: drivers may read owned scans and create initial pending entries but cannot alter verdicts; staff can read/lock evidence; scanner claims and exact-document results use separate transaction scopes. Migration 0059 protects storage-write receipts. Active owners create unsettled intents; privacy staff and assigned cleanup workers have scoped reads. Only a trusted exact-document/exact-key receipt transaction can settle an existing write, including after account closure. Consumers and staff cannot forge settlement or delete evidence. Migrations through 0059 passed isolated Neon verification with synthetic upload, scan, review, closure and cleanup through the pooled restricted role. Provider-staging rollout remains pending.
