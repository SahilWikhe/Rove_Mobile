...
The app config enables iOS location background mode and Android background-location/foreground-service permissions. The running task displays the iOS location indicator and an Android foreground-service notification. Requested updates are approximately ten seconds apart; OS delivery is not a timing guarantee.
