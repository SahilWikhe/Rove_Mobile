...
## Native iOS 3DS success

A separate September 13 sandbox request used Stripe's authentication-required test card ending 3155. The owned simulator displayed the 3D Secure 2 Test Page within the provider's browser sheet. Maestro completed the sandbox challenge and observed Payment confirmed after returning to Rove. Stripe independently reported authenticated 3DS version 2.1.0 and a $2 authorization with no capture. Authenticated HTTP cancellation returned 200; hosted release left zero captured/capturable funds and no ledger journals. This fixture is terminal.

This verifies the successful challenge path on that iOS simulator. It does not prove cancelled/failed challenges, external bank-app switching, cold-start return, Android or physical-device behavior. No real card, SMS, bank credentials or live money were used.

## Android saved payment method acceptance — September 13

Dedicated staging Auth0 login and the native CustomerSheet passed on the owned Android emulator using the existing debug APK and current Metro JavaScript. A documented sandbox Visa was saved successfully; Stripe confirmed a succeeded test SetupIntent and exactly one attached card. The card remained visible after closing and reopening CustomerSheet. Native editing/removal and its confirmation detached the same card; Stripe independently reported zero attached cards and a null customer for that payment method. No real charge or production mutation occurred. Part of Maestro form automation failed at its device transport, so remaining entry/removal used scoped emulator input and screenshots. This is saved-card acceptance, not Android charge authorization or complete ride acceptance.
