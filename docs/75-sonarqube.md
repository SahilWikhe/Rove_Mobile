...
Latest completed September 13 analysis on cec633c passed test collection and all gate conditions except new-code coverage (78.9% versus 80%). Financial restore-verifier regression tests now contribute exact-path LCOV coverage through the existing tooling suite: all 78 tests pass locally, with the verifier fully covered. Thresholds and exclusions are unchanged. A fresh hosted analysis must verify the candidate; local coverage is not a hosted gate result.

September 13 run 34791782065 stopped during server coverage collection when the refund-recovery pagination test exceeded 15 seconds. Its fixture setup now inserts the same 104 additional payments in one dependent SQL statement rather than 312 autocommits. All nine refund-reconciliation tests pass locally with coverage; pagination assertions and timeout are unchanged. This is a local test-harness correction, not proof that the hosted analysis or quality gate passes.
...
Production candidate creation and promotion require a successful SonarQube workflow run for the exact candidate SHA, in addition to CI and staging-provider evidence. Supply sonar_run_id to the manual Release readiness and Production release workflows, or the fifth positional argument (after the provider run ID) to pnpm release:check. The verifier requires the main-branch sonarqube.yml workflow and its successful SonarQube analysis job from the same current run attempt. That job waits for the configured quality gate.

Missing, failed, skipped, stale, fork or wrong-commit evidence fails closed. A rerun or status change during evidence collection also invalidates the report. The production workflow rechecks this evidence before deploying and after protected promotion approval. This uses the existing read-only GitHub token; no Sonar token is added to deployment jobs. It does not replace unresolved-issue review, device/provider acceptance or explicit production approval.
