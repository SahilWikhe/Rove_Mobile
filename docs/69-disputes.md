...
Restricted-role tests verify reconciliation and financial guards, read locks with denied writes, append-only observations, foreign-source isolation, missing MFA, revoked queue permission and pooled scope reset. Hosted verification of 0068 passed on the isolated synthetic Neon branch through the restricted pooled application role: active disputes block refund eligibility, resolved disputes release the hold, staff queue MFA/permission checks apply, and observation rewrites/deletion are denied. Deploy compatible API/worker code before the migration; provider-staging and production were not changed.
