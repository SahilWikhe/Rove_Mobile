...
## Retained application-data inventory

`GET /v1/staff/account-deletions/:id/inventory` requires verified staff MFA and current `privacy.read`. It observes counts of authored trip messages, saved places, push installations (including disabled registrations), owned support requests and driver document reservations, together with active retention holds and request withdrawal/access-closure/identity-removal state. Counts come from one SQL statement's snapshot and its observation time; a required audit records the counts before the response is returned. Failed audit persistence does not return inventory evidence. No message text, location, push token, support prose, case reference or object path is exposed.

This endpoint is read-only apart from the audit. It neither authorizes deletion nor selects retention periods, and always returns `completeErasureVerified: false`. Counts are not eligible-for-erasure counts. Authored message counts omit counterpart messages and command/outbox copies; document reservation counts do not count S3 versions, replicas or orphaned writes. Financial/audit history, provider data, caches and backups require separate inventories and policy decisions. Empty values in these five classes are not proof of complete account erasure. A withdrawn request remains inspectable for authorized review.

Use this inventory alongside closure, retention-hold and storage inspection evidence to scope an approved cleanup. No migration, cloud operation or erasure activation is introduced. Local domain/HTTP tests cover ownership scope, permission/MFA, withdrawn/missing requests, content-free responses and audit failure. Policy-driven application-data cleanup, final fulfillment and restore replay remain unfinished.

## Reviewed message-body cleanup engine

`MessageCleanup.erase` implements a bounded database cleanup transaction for up to 100 explicit, unique message IDs authored by the deletion requester. The service is disabled without a configured policy reference, which must match the reviewed input. Input also includes a review reference, creation cutoff and operation key. There is no automatic schedule or configured hosted policy. The protected runtime endpoint is described below.

Every new execution requires current staff MFA and `privacy.cleanup`, unwithdrawn consent and a closed, disabled account. It locks conversation participants before trip rows and rechecks eligibility under those locks. Active holds on the requester, original counterpart or current assigned driver block the batch, regardless of hold review date. Reported conversations remain retained. Messages must precede the reviewed cutoff and the existing 30-day messaging access window, and their trips must be terminal. The 30-day lower bound is an access safeguard, not approval of a business retention duration. Any ineligible or foreign target rejects the entire batch.

Deletion, a body-free audit and an idempotent receipt commit together. A lost-response replay returns the receipt after fresh staff authorization; it does not execute another deletion. Audit failure rolls back deletion. Counterpart-authored messages, read markers, reports, support records and financial evidence remain. Current message sending stores no bodies in command results or notification outbox payloads. This source trace does not establish absence of historical copies, device caches, exports or backups. The receipt explicitly reports `completeErasureVerified: false`.

Six local PostgreSQL tests cover scoped deletion and retry, revoked permission, missing MFA/configuration, open accounts, ownership mismatch, recent/reported messages, counterpart holds, audit rollback and a hold committed while deletion waits on the participant lock. Existing messaging tests also pass. Remaining: operational review of actual batches, approved policy configuration, hosted acceptance, other retained data classes, external copies and final fulfillment evidence. No hosted message has been erased by this work.

## Message cleanup operational endpoint

`POST /v1/staff/account-deletions/:id/messages/erase` accepts the shared strict `MessageCleanupAuthorization` contract: `policyReference`, `reviewReference`, `createdBefore` (UTC ISO timestamp), and `messageIds` (1–100 unique UUIDs). Supply `Idempotency-Key` and normal authenticated staff credentials with verified MFA/current `privacy.cleanup`. The response reports `requestId`, `removedMessages`, and `completeErasureVerified: false`. Schema/body failures return 400; authorization failures return 403; ineligible batches return a conflict. Successful retries return the original receipt without deleting additional data.

Runtime composition defaults off. Enabling requires `MESSAGE_CLEANUP_ENABLED=true` and a nonempty reviewed `MESSAGE_CLEANUP_POLICY_REFERENCE` of at most 128 characters. Invalid flags or missing policy fail configuration; a disabled endpoint returns 503 after normal authentication. Do not enable until the retention policy and exact target IDs/cutoff have been reviewed. A request must contain the configured policy reference. This operation deletes message bodies from the primary database only; retained counterpart messages and other copies remain outside its scope.

An authenticated local API test exercised anonymous/rider/staff-without-permission/staff-without-MFA rejection, duplicate-ID validation, actual synthetic message deletion, required audit and lost-response replay. The API/runtime suite passed 64 tests and cleanup domain suite passed six tests. Configuration and code are committed; no hosted flag or data was changed.
