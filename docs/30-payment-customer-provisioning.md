...
Native PaymentSheet and callback handling are wired in docs/31-native-rider-payments.md. Remaining: real sandbox/native-device verification, saved-method setup, customer deletion/retention and support-review workflows, runtime secret/configuration wiring, financial ledger/receipts/refunds, driver earnings/payouts and full production verification. A connected assistant plugin is separate from application credentials and is not evidence that a deployed app can reach Stripe.
