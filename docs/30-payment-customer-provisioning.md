...
Financial worker suites also use restricted roles. Direct checks verify foreign/unscoped mapping denial, read-only worker access, missing/wrong-source lookup clearing, and no pooled context leakage. Hosted verification of 0066 passed on the isolated synthetic Neon branch through the restricted pooled role, including provisioning retries, reconciliation, capture accounting and denied mapping mutation. Provider-staging rollout remains pending; deploy compatible API and worker code before applying this migration. Other financial tables still require their own policies.
