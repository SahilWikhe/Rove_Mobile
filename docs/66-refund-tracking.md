...
Restricted-role tests verify reconciliation/retries, source isolation, read locks with denied mutations, append-only observations, and context reset alongside refund, loss-allocation and transfer workflows. Deploy compatible API/worker code before the migration. Hosted verification of 0067 remains pending; provider-staging and production were not changed.
