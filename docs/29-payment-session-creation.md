...
Durable customer provisioning is implemented and can compose with this service; see [customer provisioning](30-payment-customer-provisioning.md). Native PaymentSheet and callback handling are implemented in docs/31-native-rider-payments.md. Still required: saved-method setup, real PaymentSheet/SCA device verification, account-specific publishable-key configuration, sandbox network verification, creation-review tooling, ledger/receipts/refunds/earnings/payouts and production composition. A plugin connection does not supply app runtime credentials automatically. Existing local synthetic settlement remains explicit; this endpoint must not be presented as a completed production payment integration.
