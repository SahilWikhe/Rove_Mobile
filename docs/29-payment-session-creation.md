...
Still required: durable customer provisioning, saved-method setup, native PaymentSheet/SCA and callback handling, account-specific publishable-key configuration, sandbox network verification, creation-review tooling, ledger/receipts/refunds/earnings/payouts and production composition. A plugin connection does not supply app runtime credentials automatically. Existing local synthetic settlement remains explicit; this endpoint must not be presented as a completed production payment integration.
