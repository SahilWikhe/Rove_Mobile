...
## Repeatable sandbox transfer observation

Use `pnpm payments:transfer:check /absolute/path/to/ignored-staging.env /absolute/path/to/approved-sandbox-transfer.json` after creating a transfer through the existing authorized staging workflow. The environment file must explicitly contain `ROVE_ENVIRONMENT=staging`, a restricted test key in `STRIPE_SECRET_KEY`, and the expected `STRIPE_ACCOUNT_ID`. The command does not merge ambient environment credentials and rejects live keys before constructing the provider.

The private JSON fixture contains `transferId`, `expectedReversedCents` and `reference`. Copy reference values from the durable authorized test operation: `operationId`, `firstAttemptAt`, `driverId`, `bindingId`, `accountId`, `chargeId`, `amountCents`, and its `payment` object (`intentId`, `rideId`, `attemptId`, `customerId`, `amountCents`). Do not invent new operation identifiers or reset the first-attempt time. Keep fixture files outside the repository and use only dedicated synthetic sandbox accounts. This command validates observations; supplying a fixture does not authorize a transfer.

First verify with `expectedReversedCents=0`. After an approved sandbox reversal, rerun with the expected reversed amount using the same operation reference and transfer ID. The actual application adapter validates platform, test mode, source charge, destination, metadata, currency, fees, movements and complete reversal history. The check additionally requires direct retrieval and operation-scoped recovery lookup to agree on transfer ID, amount, expected reversal, creation time and movement contents. A concurrent provider change fails the observation and requires a fresh check.

The command exposes only read methods and never creates customers, charges, transfers or reversals. Provider payloads and identifiers are not printed. It is intentionally separate from automatic per-commit provider checks. Local tests cover configuration guards, lookup disagreement, expected reversal and sanitized errors. No live Stripe fixture has been executed for this checkpoint: authorization, outbox delivery, lost-response mutation retries and bank-deposit acceptance still require the full staging workflow, not just this reader.
