...
export async function testDatabase() {
  ...
  const postgres = new EmbeddedPostgres({ databaseDir: await mkdtemp(join(tmpdir(), 'rove-postgres-')), port,
  ...
  catch (error) { await database.pool.end(); await postgres.stop(); throw error; }
  return { ...database, close: async () => { await database.pool.end(); await postgres.stop(); } };
}
