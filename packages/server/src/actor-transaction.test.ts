...
beforeAll(async () => {
  ...
  for (const actor of [alice, bob, driver])
  ...
  await db.pool.query('GRANT SELECT,UPDATE ON users TO rls_runtime');
  await db.pool.query('GRANT SELECT,INSERT,UPDATE,DELETE ON saved_places TO rls_runtime');
  // Temporary policy exercises the actual saved_places table without deploying an incomplete migration.
  await db.pool.query('ALTER TABLE saved_places ENABLE ROW LEVEL SECURITY');
  await db.pool.query('ALTER TABLE saved_places FORCE ROW LEVEL SECURITY');
  await db.pool.query(`CREATE POLICY owned_saved_places ON saved_places USING (
    rider_id = NULLIF(current_setting('rove.actor_id',true),'')::uuid
    AND current_setting('rove.actor_role',true)='rider') WITH CHECK (
    rider_id = NULLIF(current_setting('rove.actor_id',true),'')::uuid
    AND current_setting('rove.actor_role',true)='rider')`);
  pool = new Pool({
    host: '127.0.0.1',
