...
export class MatchingService {
  ...
  private async inspect(client: PoolClient, rideId: string): Promise<SearchRow | null> {
    ...
    const count = (
      await client.query<{ count: number }>('SELECT count(*)::int AS count FROM offers WHERE ride_id=$1', [
        rideId,
      ])
    ).rows[0]!.count;
    const pending = (
      await client.query<{ expires_at: Date }>(
        "SELECT expires_at FROM offers WHERE ride_id=$1 AND status='pending'",
        [rideId],
      )
    ).rows[0];
  ...
  async tick(rideId: string): Promise<void> {
    ...
    const ranked = (
      await Promise.all(
        nearby.map(async (candidate) => {
          const from: Place = {
            id: `driver:${candidate.id}`,
          ...
          try {
            ...
            if (
              !Number.isSafeInteger(route.durationSeconds) ||
    ...
    await transaction(this.pool, async (client) => {
      ...
      for (const candidate of ranked) {
        ...
        const expires = new Date(Math.min(this.now().getTime() + 20_000, ride.search_deadline.getTime()));
        const offer = DriverOffer.parse({
          id: offerId,
          rideId,
          ...
          pickupSeconds: candidate.seconds,
          tripSeconds: quote.durationSeconds,
          distanceMeters: quote.distanceMeters,
          estimatedEarnings: quote.estimatedDriverEarnings,
        });
        await client.query(
          'INSERT INTO offers(id,ride_id,driver_id,expires_at,snapshot) VALUES($1,$2,$3,$4,$5)',
