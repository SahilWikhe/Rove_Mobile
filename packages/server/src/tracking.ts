...
function digest(token: string) {
  if (!/^rt_[A-Za-z0-9_-]{43}$/.test(token)) throw new DomainError('TRACKING_UNAUTHORIZED', 'Open the driver app to reconnect location.', 401);
  return createHash('sha256').update(token).digest('hex');
}
const unauthorized = () => new DomainError('TRACKING_UNAUTHORIZED', 'Open the driver app to reconnect location.', 401);
...
export class TrackingService {
  ...
  async issue(actor: Actor) {
    ...
    await transaction(this.pool, async client => {
      const result = await client.query('SELECT d.online,u.disabled FROM drivers d JOIN users u ON u.id=d.id WHERE d.id=$1 FOR UPDATE OF d', [actor.id]);
      ...
      await client.query(`INSERT INTO driver_tracking_sessions(driver_id,token_hash,expires_at)
  ...
  async location(token: string, raw: unknown) {
    ...
    const now = this.now();
    if (!parsed.success) throw new DomainError('INVALID_LOCATION_SAMPLE', 'A fresh, accurate location is required.', 422);
    const sample = parsed.data;
    const age = now.getTime() - Date.parse(sample.sampledAt);
    if (age < -5000 || age > 30_000) throw new DomainError('INVALID_LOCATION_SAMPLE', 'A fresh, accurate location is required.', 422);
    return transaction(this.pool, async client => {
      // Lock order matches issue/availability: driver first, then tracking grant.
      const result = await client.query(`SELECT d.id,u.disabled,d.online FROM drivers d JOIN users u ON u.id=d.id
        JOIN driver_tracking_sessions s ON s.driver_id=d.id WHERE s.token_hash=$1 FOR UPDATE OF d`, [hash]);
      const driver = result.rows[0];
      if (!driver?.online || driver.disabled) throw unauthorized();
      const grant = (await client.query('SELECT expires_at,sampled_at FROM driver_tracking_sessions WHERE driver_id=$1 AND token_hash=$2 FOR UPDATE', [driver.id, hash])).rows[0];
      ...
      await client.query('UPDATE driver_tracking_sessions SET sampled_at=$2 WHERE driver_id=$1', [driver.id, sampledAt]);
      await client.query('UPDATE drivers SET location=$2,location_at=$3,location_sequence=location_sequence+1 WHERE id=$1',
