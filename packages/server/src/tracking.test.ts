...
beforeEach(async () => {
  ...
  await database.db.insert(users).values({ id: actor.id, subject: actor.id, name: 'Synthetic driver', role: 'driver' });
  await database.db.insert(drivers).values({ id: actor.id, online: true, approved: true, payoutReady: true, eligibilityExpiresAt: new Date(now.getTime() + 86_400_000) });
});
test('grants store only a hash and rotate instead of keeping multiple active devices', async () => {
  ...
  await expect(tracking.location(first.token, sample())).rejects.toMatchObject({ code: 'TRACKING_UNAUTHORIZED' });
...
test('out-of-order and duplicate callbacks do not overwrite location or refresh freshness', async () => {
  ...
  expect(await tracking.location(grant.token, { ...sample(), sampledAt: original, coordinate: { latitude: 0, longitude: 0 } })).toEqual({ accepted: false });
  const row = (await database.pool.query('SELECT location,location_at,location_sequence FROM drivers')).rows[0];
...
test('expired grants, disabled accounts and offline drivers cannot upload', async () => {
  ...
  await expect(tracking.location(grant.token, sample())).rejects.toMatchObject({ code: 'TRACKING_UNAUTHORIZED' });
  const renewed = await tracking.issue(actor);
  await database.pool.query('UPDATE users SET disabled=true');
  await expect(tracking.location(renewed.token, sample())).rejects.toMatchObject({ code: 'TRACKING_UNAUTHORIZED' });
...
test('rejects stale, inaccurate, impossible and future samples without touching freshness', async () => {
  ...
  ]) await expect(tracking.location(grant.token, invalid)).rejects.toMatchObject({ code: 'INVALID_LOCATION_SAMPLE' });
...
test('concurrent duplicate delivery commits only one update', async () => {
  const grant = await tracking.issue(actor);
  const results = await Promise.all([tracking.location(grant.token, sample()), tracking.location(grant.token, sample())]);
  expect(results.filter(result => result.accepted)).toHaveLength(1);
  expect((await database.pool.query('SELECT location_sequence FROM drivers')).rows[0].location_sequence).toBe(1);
});
