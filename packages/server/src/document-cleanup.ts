...
export class DocumentCleanup {
  ...
  async inspectUploads(actor: Actor, documentId: string, after?: string) {
    ...
    return transaction(this.pool, async (c) => {
      ...
      const target = (
        await c.query<{ driver_id: string }>('SELECT driver_id FROM driver_documents WHERE id=$1', [
          documentId,
        ])
      ).rows[0];
      if (!target) throw new DomainError('NOT_FOUND', 'Document not found.', 404);
      await bindUserRead(c, target.driver_id);
  ...
  async approve(actor: Actor, planId: string, raw: unknown, key: string) {
    ...
    return command(
      ...
      async (c) => {
        ...
        await c.query("SELECT set_config('rove.outbox_cleanup_plan',$1,true)", [planId]);
        await c.query(
