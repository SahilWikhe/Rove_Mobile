...
export async function bindActorIdentity(
  ...
  await client.query(`SELECT set_config('rove.payment_review_intake','',true),set_config('rove.payment_review_ack','',true),set_config('rove.actor_id','',true),
  ...
  await bindUserRead(client, parsed.data.id);
