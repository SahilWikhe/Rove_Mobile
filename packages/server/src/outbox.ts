...
export class OutboxWorker {
  ...
  async runOnce(maxJobs = 10): Promise<{ processed: number; failed: number }> {
    ...
    for (let index = 0; index < Math.min(Math.max(maxJobs, 1), 50); index++) {
      ...
      const row = (
        await outboxTransaction(this.pool, { kind: 'claim', token, at: claimAt }, (client) =>
      ...
      try {
        ...
        if (!handler)
          throw new DomainError('UNKNOWN_JOB_TYPE', 'No handler is registered for this event.', 422);
        await handler({
          id: row.id,
          topic: row.topic,
        ...
        await outboxTransaction(this.pool, { kind: 'ack', id: row.id, token }, (client) =>
          client.query(
            'UPDATE outbox SET completed_at=$3,locked_until=NULL,lease_token=NULL,last_error_code=NULL WHERE id=$1 AND lease_token=$2',
            [row.id, token, this.now()],
          ),
        );
        processed++;
      } catch (error) {
        ...
        const retryMs =
          Math.min(300_000, 1000 * 2 ** Math.min(row.attempts, 8)) * (0.8 + 0.4 * this.random());
        await outboxTransaction(this.pool, { kind: 'ack', id: row.id, token }, (client) =>
          client.query(
            'UPDATE outbox SET locked_until=NULL,lease_token=NULL,last_error_code=$3,available_at=$4,dead_letter_at=$5 WHERE id=$1 AND lease_token=$2',
