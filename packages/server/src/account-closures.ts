...
export class AccountClosures {
  ...
  async authorize(actor: Actor, requestId: string, raw: unknown, key: string) {
    ...
    return command(this.pool, actor.id, key, { action: 'account.close', requestId, ...input }, async (c) => {
      ...
      await c.query("SELECT set_config('rove.payment_attempt_closure',$1,true)", [ownerId]);
      ...
      await bindLedgerOwner(c, ownerId);
      ...
      await c.query("SELECT set_config('rove.transfer_closure_owner',$1,true)", [ownerId]);
      ...
      await c.query("SELECT set_config('rove.refund_closure_owner',$1,true)", [ownerId]);
      ...
      await c.query(
        "SELECT set_config('rove.user_close_write',(to_jsonb(u)||jsonb_build_object('disabled',true))::text,true) FROM users u WHERE id=$1",
        [ownerId],
      );
      await c.query('UPDATE users SET disabled=true WHERE id=$1', [ownerId]);
      await bindDriverMutation(c, ownerId, 'closure');
      await c.query(
      ...
      await bindTrackingClosure(c, ownerId);
  ...
  async retryIdentity(actor: Actor, requestId: string, key: string) {
    ...
    return command(this.pool, actor.id, key, { action: 'account.identity-retry', requestId }, async (c) => {
      ...
      await c.query("SELECT set_config('rove.outbox_identity_request',$1,true)", [requestId]);
