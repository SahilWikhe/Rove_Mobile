...
export class SavedPlaceService {
  ...
  async list(actor: Actor) {
    ...
    const result = await actorTransaction(this.pool, actor, (client) =>
      client.query('SELECT kind,place_id AS "placeId" FROM saved_places WHERE rider_id=$1 ORDER BY kind', [
        actor.id,
      ]),
    );
  ...
  async resolve(actor: Actor, kind: SavedPlaceKind) {
    rider(actor);
    const result = await actorTransaction(this.pool, actor, (client) =>
      client.query<{ place_id: string }>('SELECT place_id FROM saved_places WHERE rider_id=$1 AND kind=$2', [
        actor.id,
        kind,
      ]),
    );
